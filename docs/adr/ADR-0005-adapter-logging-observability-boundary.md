...
Forensic Analytics needs operational logs for inbound requests, CLI commands and server lifecycle events. The logging source material inspected from the SCXML example project used Spring AOP, AspectJ, SLF4J MDC and annotation-driven method logging.
...
Inbound adapters and bootstrap code may depend on this module. Domain and application code must not depend on it.
...
- Logging is explicit at adapter boundaries instead of implicit through method weaving.
