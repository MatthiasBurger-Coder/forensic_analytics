...
- repository-to-BTM worker-dispatch or job-graph state;
- direct database access.

Inbound communication:

- REST/OpenAPI from frontend, CLI and external clients.
- public gRPC BTM delivery requests from plugins or external clients after
  Slice 02 defines the contract.

Outbound communication:

- REST/gRPC to ingestion, repository analysis, analysis store, BTM generation,
  graph/replay and report services after contracts exist.
- repository-to-BTM orchestration requests only through the approved owner API
  defined by the workflow Slice 11 contract/readiness bridge.
- BTM owner API or approved artifact owner path for generated byte retrieval
  after contracts exist. Gateway never stores canonical artifact bytes.
...
- rule ID stability;
- generated BTM artifact bytes until an explicit byte-handoff or object-store
  ownership contract transfers byte custody. Analysis Store registration
  transfers catalog metadata only.
...
- generated artifact metadata to Analysis Store through owner APIs or events
  after contracts exist;
- generated artifact bytes through BTM Generation owner APIs or an approved
  artifact-storage handoff after the relevant contract slice defines the path.
