...
| Repository-source database Settings | `repository-source-service` owns active PostgreSQL configuration status and candidate validation semantics. `query-report-api-service` owns only the operator-protected public facade and does not persist credentials or read repository-source tables. | Public Settings API delegates to repository-source owner gRPC methods and returns sanitized status plus restart-required apply semantics | S08 owner clear |
