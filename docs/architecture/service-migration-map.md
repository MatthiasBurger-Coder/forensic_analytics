...
|---|---|
| `forensic-analytics-domain` | Split into service-owned domain models during Slice 14/15 only after caller parity is verified; no shared domain module in target services |
...
| `forensic-analytics-cli` | Gateway/public API client adapter after Slice 13 if CLI remains in repository scope |
| `forensic-analytics-testbed` | Retain as monolith test evidence until Slice 14/15 decides parity or retirement; do not share as service fixture module |
...
4. Define artifact-byte and instrumentation-target ownership contracts in
   Slice 03.
5. Add service-local implementations one service slice at a time.
6. Verify service independence before routing runtime behavior.
7. Remove obsolete monolith paths only after replacement evidence exists.
