...
|---|---|---|
| Target architecture is ahead of verified runtime readiness | `docs/arc42/08-crosscutting-concepts/architecture-source-maps/target-microservices-architecture.md`, `docs/arc42/08-crosscutting-concepts/architecture-source-maps/current-state.md`, `docs/arc42/08-crosscutting-concepts/architecture-source-maps/service-roots.md` distinguish target service names, transitional evidence and missing production readiness | Keep target architecture, current implementation evidence and readiness claims separate; require independent build, start, test, configuration, healthcheck, container and deployment evidence before claiming production microservices |
| Distributed monolith risk | `docs/arc42/08-crosscutting-concepts/architecture-source-maps/microservice-governance.md` warns that a technical module is not a microservice; `docs/arc42/08-crosscutting-concepts/architecture-source-maps/current-coupling-map.md` keeps historical coupling evidence | Keep service boundaries business-owned, contract-first and data-owner-specific; block shared Java implementation, shared DTO/domain modules and direct cross-service database access |
| Wrong service cut risk | `docs/arc42/08-crosscutting-concepts/architecture-source-maps/service-boundaries.md` defines explicit service ownership and non-scope for repository source, ingestion, JavaParser, Joern, orchestration, query/report, CLI, observability and testbed boundaries | Route future service-boundary changes through Senior System Architect, microservice review, contract governance, data-ownership review and test-impact review |
| Early or unstable gRPC contract stabilization | `docs/arc42/08-crosscutting-concepts/architecture-source-maps/service-communication-matrix.md` and `docs/arc42/08-crosscutting-concepts/architecture-source-maps/contract-versioning.md` classify current contract files as transitional evidence in several areas | Keep REST/gRPC changes contract-first under ADR-0010; verify producer, consumer, semantics, compatibility and contract tests before implementation depends on changed communication behavior |
| Repository-source PostgreSQL could be mistaken for shared analytics persistence | ADR-0024 and `docs/arc42/08-crosscutting-concepts/architecture-source-maps/data-ownership.md` bound PostgreSQL to repository checkout workspace metadata only | Keep PostgreSQL one-writer and service-owned by `repository-source-service`; leave broader canonical Analytics persistence, graph, vector, report and LLM package storage decisions open until separate ADR coverage exists |
...
| Legacy source-tree and predecessor names may be misread as current runtime units | `docs/arc42/08-crosscutting-concepts/architecture-source-maps/legacy-reference-classification.md`, `docs/arc42/08-crosscutting-concepts/architecture-source-maps/monolith-runtime-isolation.md` and ADR-0022 classify legacy `forensic-analytics-*` source trees as historical after S05 | Use historical, predecessor, rollback or compatibility wording only when verified; do not present retired source trees as active runtime modules |
| Optional or placeholder service roots may be overclaimed | `docs/arc42/08-crosscutting-concepts/architecture-source-maps/current-state.md` and `docs/arc42/08-crosscutting-concepts/architecture-source-maps/service-roots.md` mark graph-replay and report-generation as planned or optional evidence | Keep optional services outside mandatory readiness claims until a later requirement creates implementation, contracts, ownership, tests and deployment evidence |
