...
public final class PostgresRepositorySourcePersistenceAdapter implements
    ...
    public PostgresRepositorySourcePersistenceAdapter(String jdbcUrl, String username, String password, String schema) {
        this(schema, driverConnectionFactory(jdbcUrl, username, password));
    }

    public PostgresRepositorySourcePersistenceAdapter(String schema, ConnectionFactory connectionFactory) {
        this.schema = requireSchema(schema);
    ...
    public RepositoryPreparation save(RepositoryPreparation preparation) {
        ...
        try (var connection = connection()) {
            try (var statement = connection.prepareStatement("""
                ...
                ON CONFLICT (analysis_run_id, source_snapshot_id) DO UPDATE SET
                    workspace_id = EXCLUDED.workspace_id,
                    workspace_branch_id = EXCLUDED.workspace_branch_id,
                    repository_url = EXCLUDED.repository_url,
                    repository_provider = EXCLUDED.repository_provider,
                    repository_attributes_json = EXCLUDED.repository_attributes_json,
                    requested_branch = EXCLUDED.requested_branch,
                    branch_required = EXCLUDED.branch_required,
                    requested_commit = EXCLUDED.requested_commit,
                    commit_required = EXCLUDED.commit_required,
                    resolved_commit = EXCLUDED.resolved_commit,
                    checkout_status = EXCLUDED.checkout_status,
                    checkout_diagnostics_json = EXCLUDED.checkout_diagnostics_json,
                    shallow_clone = EXCLUDED.shallow_clone,
                    checkout_duration_millis = EXCLUDED.checkout_duration_millis,
                    partial_clone = EXCLUDED.partial_clone,
                    sparse_checkout = EXCLUDED.sparse_checkout,
                    workspace_status = EXCLUDED.workspace_status,
                    source_snapshot_completeness = EXCLUDED.source_snapshot_completeness,
                    source_snapshot_json = EXCLUDED.source_snapshot_json,
                    source_roots_json = EXCLUDED.source_roots_json,
                    diagnostics_json = EXCLUDED.diagnostics_json,
                    safe_attributes_json = EXCLUDED.safe_attributes_json,
                    created_at = EXCLUDED.created_at,
                    updated_at = EXCLUDED.updated_at
                """.formatted(table("repository_preparation")))) {
    ...
    public Optional<RepositoryWorkspaceBranch> findBranch(WorkspaceId workspaceId, WorkspaceBranchId workspaceBranchId) {
        try (var connection = connection();
             var statement = connection.prepareStatement("""
                 ...
                 """.formatted(table("workspace_branch")))) {
            statement.setString(1, workspaceId.value());
    ...
    public Optional<RepositorySourceIdempotencyRecord> find(String operation, String idempotencyKey) {
        try (var connection = connection();
             var statement = connection.prepareStatement("""
                 ...
                 """.formatted(table("idempotency_record")))) {
    ...
    public RepositorySourceIdempotencyRecord save(RepositorySourceIdempotencyRecord record) {
        try (var connection = connection();
             var statement = connection.prepareStatement("""
                 ...
                 ON CONFLICT (idempotency_key, operation) DO UPDATE SET
    ...
    private static ConnectionFactory driverConnectionFactory(String jdbcUrl, String username, String password) {
        Objects.requireNonNull(jdbcUrl, "jdbc url must not be null");
        var safeUsername = Objects.requireNonNullElse(username, "");
        var safePassword = Objects.requireNonNullElse(password, "");
        loadPostgreSQLDriver();
        return () -> DriverManager.getConnection(jdbcUrl, safeUsername, safePassword);
    }
    ...
    private static void loadPostgreSQLDriver() {
        try {
            ...
            throw new IllegalStateException("PostgreSQL JDBC driver is not available", error);
        }
    }

    private static String requireSchema(String schema) {
        if (schema == null || schema.isBlank() || !schema.matches("[A-Za-z_][A-Za-z0-9_]*")) {
            throw new IllegalArgumentException("PostgreSQL schema must be a simple SQL identifier");
        }
        return schema.trim();
    }
    ...
    private void deleteBranches(Connection connection, WorkspaceId workspaceId) throws SQLException {
        try (var statement = connection.prepareStatement("DELETE FROM " + table("workspace_branch") + " WHERE workspace_id = ?")) {
            statement.setString(1, workspaceId.value());
            statement.executeUpdate();
        }
    }
    ...
    private void saveBranch(Connection connection, RepositoryWorkspaceBranch branch) throws SQLException {
        try (var statement = connection.prepareStatement("""
            ...
            ON CONFLICT (workspace_branch_id) DO UPDATE SET
                workspace_id = EXCLUDED.workspace_id,
    ...
    private SourceSnapshot sourceSnapshot(
        ...
        if (json == null || json.isBlank()) {
            throw new IllegalStateException("PostgreSQL source snapshot metadata is missing");
        }
