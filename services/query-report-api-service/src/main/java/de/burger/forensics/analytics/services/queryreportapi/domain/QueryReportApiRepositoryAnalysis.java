...
public final class QueryReportApiRepositoryAnalysis {
    ...
    private QueryReportApiRepositoryAnalysis() {
    }
    ...
    public record RepositoryToBtmStatus(
        String analysisRunId,
        String branch,
        String commit,
        String resolvedCommit,
        String sourceSnapshotStatus,
        String status,
        String workflow,
        String createdAt,
        List<Diagnostic> diagnostics
    ) {
        public RepositoryToBtmStatus {
            analysisRunId = requireText(analysisRunId, "analysis run id");
            branch = optionalNullableSafeText(branch, "branch");
            commit = optionalNullableSafeText(commit, "commit");
            resolvedCommit = optionalNullableSafeText(resolvedCommit, "resolved commit");
            sourceSnapshotStatus = optionalNullableSafeText(sourceSnapshotStatus, "source snapshot status");
            status = requireText(status, "status");
            workflow = optionalNullableSafeText(workflow, "workflow");
            createdAt = optionalNullableSafeText(createdAt, "created at");
            diagnostics = List.copyOf(Objects.requireNonNullElse(diagnostics, List.of()));
        }
    }
    ...
    private static String optionalNullableSafeText(String value, String name) {
        if (value == null || value.isBlank()) {
            return null;
        }
        return optionalSafeText(value, name);
    }
